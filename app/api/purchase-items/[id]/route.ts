...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        const allowed = ['name', 'quantity', 'unit', 'brand', 'notes', 'checked', 'has_problem', 'problem_notes'];
        const payload: Record<string, unknown> = Object.fromEntries(
            Object.entries(updates).filter(([k]) => allowed.includes(k))
        );

        // Setar checked_by e checked_at ao marcar como concluído
        ...
        if (error) {
            console.error('[PUT /api/purchase-items/[id]]', error);
            return NextResponse.json({ error: error.message }, { status: 500 });
        }

        return NextResponse.json(data);
    } catch (error: unknown) {
        console.error('[PUT /api/purchase-items/[id]] Erro inesperado:', error);
