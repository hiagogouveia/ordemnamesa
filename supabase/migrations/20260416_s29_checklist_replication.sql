-- Fase 5 — Replicação de checklists entre unidades da mesma account.
--
-- Adiciona colunas de linhagem em public.checklists:
--   source_checklist_id        → checklist imediatamente anterior na cadeia
--   root_source_checklist_id   → origem canônica (usada para idempotência)
--
-- Cria RPC public.replicate_checklists(uuid[], uuid[]).
...
-- ── RPC ──────────────────────────────────────────────────────────
-- Retorna uma linha por par (source, target) com o resultado:
--   status ∈ ('created','skipped','error')
--
-- SECURITY DEFINER: auth.uid() é obtido do caller (chamar com JWT do usuário,
-- nunca com service_role, senão auth.uid() = NULL e a RPC rejeita).
...
BEGIN
    ...
    SELECT count(DISTINCT r.account_id), min(r.account_id)
