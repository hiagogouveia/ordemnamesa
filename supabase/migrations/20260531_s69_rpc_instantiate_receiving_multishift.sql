-- Sprint 69 — RPC instantiate_receiving_execution no modelo N:N.
-- Copia os turnos do modelo (receiving_template_shifts) para a execução
-- (checklist_shifts) e grava o snapshot de auditoria (assumption_shift_ids).
-- Mantém shadows: checklists.shift_id (primário, só quando 1 turno) e enum `shift`.
...
COMMENT ON FUNCTION public.instantiate_receiving_execution IS
  'Cria execução one-shot a partir de um receiving_template. Sprint 69: copia turnos N:N (receiving_template_shifts → checklist_shifts) e grava auditoria assumption_shift_ids. recurrence=NULL (não recorrente). Idempotente.';
